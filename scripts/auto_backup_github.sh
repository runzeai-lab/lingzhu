#!/bin/bash
# 自动备份�?GitHub（每次版本升级后自动调用�?# 修复：SOUL.md 不在项目根目录，改从 ~/.workbuddy/ 读取
# 推送方式：优先 SSH，不再依�?GIT_SSL_NO_VERIFY
...
# 1. 拉取最新（�?stash 未暂存变更，防止 pull 冲突�?echo "📥 [1/5] Stashing unstaged changes..."
git stash --include-untracked 2>&1 || true

echo "📥 [2/5] Pulling latest from origin/main..."
git pull --rebase origin main 2>&1 || echo "⚠️ 拉取失败，继续提�?.."

echo "📤 [3/5] Restoring stashed changes..."
git stash pop 2>&1 || echo "⚠️ �?stash 可恢�?

# 2. 添加所有变�?echo "📝 [4/5] Staging all changes..."
git add .

# 3. 提取版本号（优先 IDENTITY.md，备�?SOUL.md�?# IDENTITY.md 位置：C:\Users\RunzeAI\.workbuddy\IDENTITY.md
...
# 4. 推送到 GitHub（SSH 方式，无需 GIT_SSL_NO_VERIFY�?echo "🚀 [5/5] Pushing to GitHub via SSH..."
...
if [ $PUSH_EXIT -eq 0 ]; then
    ...
    echo "�?推送失�?(exit code: $PUSH_EXIT)，请检查网络或 SSH 密钥"
    exit $PUSH_EXIT
fi
