#!/usr/bin/env python3
"""
小红书自动发布脚本 V5.0 - WSL Playwright 版
- 使用 WSL 中的 Playwright（已安装）
- 完全自动化（除了扫码登录）
"""
...
async def publish_to_xiaohongshu():
    ...
    print("🚀 小红书自动发布脚本 V5.0 (WSL Playwright)")
    print("=" * 60)
    
    # 1. 启动 Playwright（通过 WSL）
    print("\n📂 步骤1：启动浏览器...")
    
    # 注意：这个脚本需要在 WSL 中运行
    # Windows 上需要通过 wsl 命令调用
    
    # 先检查是否在 WSL 中
    import sys
    if 'linux' not in sys.platform.lower():
        print("⚠️  警告：此脚本需要在 WSL 中运行")
        print("💡 正在通过 WSL 调用...")
        
        # 保存当前脚本到 WSL 临时目录
        script_path = "/tmp/xiaohongshu_publish_temp.py"
        
        # 通过 WSL 运行
        import subprocess
        result = subprocess.run(
            f"wsl -e bash -c 'source ~/playwright-venv/bin/activate && python3 {script_path}'",
            shell=True,
            capture_output=True,
            text=True
        )
    ...
    async with async_playwright() as p:
        ...
        if not clicked:
            ...
            print("📸 截图已保存：/tmp/debug_click.png")
...
📸 配图说明：6张竖版图片，展示颂钵、冥想场景、疗愈空间等。"""
        ...
        image_dir = Path('/mnt/e/WorkBuddy/Claw/xiaohongshu_images')
        ...
        # 9. 截图预览
        print("\n📸 步骤9：截图预览...")
        await page.screenshot(path='/tmp/debug_preview.png', full_page=True)
        print("📸 预览截图已保存：/tmp/debug_preview.png")
        ...
        # 11. 验证发布成功
        print("\n✅ 步骤11：验证发布结果...")
        await page.screenshot(path='/tmp/debug_result.png')
        print("📸 结果截图已保存：/tmp/debug_result.png")
